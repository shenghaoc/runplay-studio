...
struct SwiftRouteMetricScaleBucketOracle {
    ...
    static func assign(
        ...
        precondition(metricValues.count == weightsMeters.count)
        ...
        if validCount >= minimumValidIntervalCount,
           ...
           abs(upper - lower) + 1e-12 >= max(0, minimumScaleSpan) {
            scale = Scale(lowerBound: lower, median: median, upperBound: upper)
        } else {
            scale = nil
        }
    ...
    private static func normalize(value: Double, scale: Scale) -> Double {
