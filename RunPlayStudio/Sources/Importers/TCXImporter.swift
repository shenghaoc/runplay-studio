...
struct TCXImporter: WorkoutImporting {
    ...
    private func parseTCX(_ xml: String, sourceURL: URL) throws -> RunWorkout {
        ...
        let validPoints = rawResult.trackpoints.filter { tp in
            tp.latitude.isFinite && tp.longitude.isFinite &&
            !tp.latitude.isNaN && !tp.longitude.isNaN &&
            tp.latitude >= -90 && tp.latitude <= 90 &&
            tp.longitude >= -180 && tp.longitude <= 180
        }
        ...
        for (index, raw) in validPoints.enumerated() {
            ...
            if index > 0 {
                ...
                let to = CLLocation(latitude: raw.latitude, longitude: raw.longitude)
