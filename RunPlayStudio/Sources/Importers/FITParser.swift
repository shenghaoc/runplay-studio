...
enum FITError: Error, LocalizedError {
    case emptyFile
    case invalidHeader
...
struct FITRecordMessage {
    var timestamp: UInt32?
    var positionLat: Int32?      // semicircles
    var positionLong: Int32?     // semicircles
    var altitude: UInt16?        // scaled
    var enhancedAltitude: UInt16?
    var distance: UInt32?        // scaled
    var speed: UInt16?           // scaled
    var enhancedSpeed: UInt16?
    var heartRate: UInt8?
    var cadence: UInt8?
}
...
struct FITParser {
    ...
    static func parse(data: Data) throws -> [FITRecordMessage] {
...
extension FITParser {
    ...
    static func timestampToDate(_ timestamp: UInt32) -> Date {
        Date(timeIntervalSince1970: fitEpoch + Double(timestamp))
    }

    /// Convert scaled altitude to meters.
    static func scaledAltitudeToMeters(_ scaled: UInt16) -> Double {
        (Double(scaled) / altitudeScale) - altitudeOffset
    }

    /// Convert scaled distance to meters.
    static func scaledDistanceToMeters(_ scaled: UInt32) -> Double {
        Double(scaled) / distanceScale
    }

    /// Convert scaled speed to m/s.
    static func scaledSpeedToMPS(_ scaled: UInt16) -> Double {
