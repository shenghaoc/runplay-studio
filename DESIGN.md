...
### Navigation
- **Sidebar:** Native `.sidebar` list style with one shared selection for Library destinations and bounded workout rows. Library lists **All Runs**, **Personal Heatmap**, and
**Trends**. Workout lists use capped **Favourites** and **Recent** sections (plus **Selected Run** when needed), not the full library. Rows use one leading symbol, one title line, and one compact metadata line.
