...
The app uses one SwiftUI MapKit view with flat 2D and realistic-elevation 3D presentations. MapKit loads map tiles and context from Apple map services over the network. **Map loading is the one network activity in the app** — it is handled entirely by Apple's MapKit framework and is subject to Apple's privacy policies. Requests cover the map region derived from your route coordinates, so the general area of your route is visible to Apple Maps. No workout file data, metrics, heart rate, or account information is included in map requests.
