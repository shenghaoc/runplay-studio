...
- [x] **Migrate route-metric scale/bucket work to C++23**: one allocation-free bulk call per non-solid profile performs deterministic distance-weighted lower/median/upper scale construction, numeric normalization, bucket assignment, coverage accumulation, and numeric summary construction. Swift retains raw extraction, smoothing, localized labels, public profiles, availability/caching, Platform line coalescing, cancellation, diagnostics, UI, and persistence. Exact native/oracle/end-to-end parity is required; no schema, analysis-version, normalization-version, importer, or public API change. Compatibility corrections keep the full public Swift `Int` `bucketCount` domain (`std::int64_t`), positive-infinite valid coverage from extreme finite weights, individually positive-infinite weights as valid but not quantile-eligible, and a no-sort fast path when a scale is known to be impossible after the read-only validation pass.
