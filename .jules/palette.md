...
## 2026-09-23 - Menu Accessibility Labels Leak Symbol Names to the Item Chooser
**Learning:** A macOS `Menu` with an `.accessibilityLabel` that differs from its visible title is an AppKit menu button whose AXDescription is copied from the label icon's `NSImage.accessibilityDescription`. For an SF Symbol without one, AppKit generates a description from the symbol name, and VoiceOver's Item Chooser reads it, although focus does not. SwiftUI modifiers on the `Image` (`accessibilityHidden`, `accessibilityLabel`) do not change it, and an empty `NSImage` description counts as missing. Check the element's AX attributes before assuming a SwiftUI modifier works.
**Action:** For such a menu's icon, use `DecorativeMenuSymbol.image(systemName:)`, which sets a single-space description. A menu without a label override does not need it.

## 2026-09-23 - A Labelled Picker Is Already Named
**Learning:** A macOS menu `Picker` with a visible label exposes an AXPopUpButton whose AXTitleUIElement is that label. An explicit `.accessibilityLabel` adds an AXDescription too, and VoiceOver's Item Chooser reads both: "All Time Date range Date range pop up button" (#195). `labelsHidden()` plus an explicit label also names the control twice, both inside AXDescription (#160).
**Action:** Name each picker from one source. A picker with a visible label gets no `.accessibilityLabel`. When the label comes and goes with the layout, use `accessibilityLabel(_:isEnabled:)` so the explicit label applies only when the label is absent. With no visible label, omit the label view instead of using `labelsHidden()`. Verify with the Item Chooser or an AX attribute dump.
