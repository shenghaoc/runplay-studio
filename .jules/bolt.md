...
**Learning:** Calling sequence methods like `.compactMap(\.primaryPace) + .compactMap(\.comparisonPace)`, or using `.enumerated().compactMap { ... }` generates multiple intermediate O(N) arrays and closure call overhead. For high-frequency rendering and data projection paths (such as `ComparisonChartView` chart model rebuilding and `RouteProjectionService` iteration), these chained transformations create significant unnecessary ARC churn.
**Action:** When extracting or accumulating data from large arrays (especially on hot UI or backend calculation paths), avoid combining multiple mapped arrays. Instead, pre-allocate the target array using `.reserveCapacity` and populate it using a single manual, inline `for` loop to entirely avoid closure call overhead and intermediate array allocations.
