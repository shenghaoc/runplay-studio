...
## 2026-08-01 - Avoid Computed Properties with Sequence Reductions in Parsers
**Learning:** `FITDefinitionMessage.totalDataSize` was implemented as a computed property using `fields.reduce(0) { ... } + developerFields.reduce(0) { ... }`. Because this property is accessed multiple times for every single data message parsed in a binary stream, the closures were called and sequences evaluated millions of times during file import, creating massive redundant computation and overhead.
**Action:** When a sequence reduction value (like total size) is immutable once defined, pre-compute it in the `init` function using standard `for` loops and store it as a `let` property instead of a computed property. This turns millions of sequence evaluations into a single O(1) property access.
