...
## 2026-08-06 - Avoid String Array Allocation in Hex Digest Generation
**Learning:** `ContentHasher.sha256Hex` converted SHA-256 digests with `digest.map { String(format: "%02x", $0) }.joined()`. That builds an intermediate `[String]` of 32 short strings plus format-parser overhead on every fingerprint. Archive/import fingerprinting is dominated by I/O and SHA-256 itself, but this fixed-size tail still creates avoidable ARC and formatting churn whenever many files are fingerprinted in a batch.
**Action:** When converting fixed-size digests to lowercase hex, avoid `map`/`joined`/`String(format:)`. Prefer a static ASCII nibble table (`[UInt8]` of `0-9a-f`) and write both nibbles into one reserved UTF-8 buffer (e.g. `String(unsafeUninitializedCapacity:)`) so the result is a single string with no intermediate heap objects per byte.
