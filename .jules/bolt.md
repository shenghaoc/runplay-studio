...
## 2026-08-01 - O(N) Array Allocations During Map Snapshot Composition
...
## 2026-08-04 - Eliminate Multi-Map Array Allocations on Hot Paths
**Learning:** Chaining mapped arrays — e.g. `.compactMap(\.primaryPace) + .compactMap(\.comparisonPace)`, or `.enumerated().compactMap { ... }` that materializes index/value pairs — allocates throwaway O(N) buffers and pays closure call overhead. Hot UI and projection paths (`ComparisonChartView` accessibility rebuild, `RouteProjectionService` filtering, synthetic video map prep) are especially sensitive. A single 1:1 `.map` mainly costs the closure, not an extra intermediate buffer beyond the result.
**Action:** When extracting or accumulating over large route arrays, prefer `reserveCapacity` plus one inline `for` loop instead of multi-map/`+` chains. For first/last non-nil in sparse optional arrays, scan with `for case let x?` rather than `compactMap({ $0 }).first`/`last`, which allocates a full compacted copy.

## 2026-08-05 - Avoid String Array Allocation in Hex Digest Generation
**Learning:** `ContentHasher.sha256Hex` generated lowercase hexadecimal digest strings using `digest.map { String(format: "%02x", $0) }.joined()`. This is a hot path (e.g., when validating/hashing workout archives). Calling `.map` and returning strings creates an intermediate `[String]` array equal to the hash size (32 strings for SHA-256), creating unnecessary string allocation, ARC object churn, and formatting overhead.
**Action:** When converting byte sequences like cryptographic hashes to hex strings, avoid `map.joined` and `String(format:)`. Instead, allocate a target string with `reserveCapacity` and use an inline `for` loop combined with a static character lookup array (e.g. `private static let hexChars = Array("0123456789abcdef")`) to avoid object allocations entirely.
