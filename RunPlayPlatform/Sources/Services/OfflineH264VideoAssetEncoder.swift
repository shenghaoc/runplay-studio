...
/// Renders frames on demand into a caller-owned BGRA pixel buffer.
///
/// Implementations must not retain frames or pixel buffers, must not hop to
/// the main actor for rendering, and must not escape locked pixel-buffer memory.
public protocol OfflineVideoFrameProviding: Sendable {
...
/// Low-level H.264 MP4 writer with temporary-file transaction and exact media
/// validation. Domain-agnostic: knows nothing about workouts, routes, or maps.
public struct OfflineH264VideoAssetEncoder: Sendable {
    public init() {}

    private var fileManager: FileManager { .default }

    /// Encode frames from `provider` into a validated file-backed MP4.
    public func export(
        frameCount: Int,
        policy: WorkoutVideoExportPolicy,
        destinationURL: URL,
        expectedOutputDurationSeconds: Double,
        progress: @Sendable (WorkoutVideoExportProgress) async -> Void,
        provider: any OfflineVideoFrameProviding
        ...
        guard provider.frameCount == frameCount else {
            throw WorkoutVideoExportError.invalidConfiguration(
                "Frame provider count \(provider.frameCount) does not match plan \(frameCount)"
            )
        }
        ...
        do {
            ...
            try await validateOutput(
                ...
                policy: policy,
                frameCount: frameCount,
                expectedOutputDurationSeconds: expectedOutputDurationSeconds
            )
            ...
            let result = WorkoutVideoExportResult(
                ...
                fileSizeBytes: size,
                outputDurationSeconds: expectedOutputDurationSeconds,
                frameCount: frameCount,
                width: policy.width,
    ...
    private func encode(
        ...
        for frameIndex in 0..<frameCount {
            ...
            do {
                try provider.renderFrame(at: frameIndex, into: buffer)
    ...
    private func validateOutput(
        ...
        let expected = expectedOutputDurationSeconds
    ...
    private func validateFrameSamples(
        ...
        guard samplesRead == frameCount else {
            throw WorkoutVideoExportError.validationFailed(
                "Frame count \(samplesRead) does not match \(frameCount)"
            )
        }
        ...
        let finalFrameTime = Double(frameCount - 1) / Double(policy.framesPerSecond)
        let sampleTimes = [
            CMTime.zero,
            CMTime(
